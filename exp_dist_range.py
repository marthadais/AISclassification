...
dist_size = 21 # fixed number of observations
nc_size = 26 # number of clusters to be tested
eval = {}
...
for dist_eps in range(2, dist_size):
# for dist_eps in [1/1000, 10/1000, 100/1000]:
    dataset = dataset_raw.copy()
    folder = f'./results/dist_{dist_eps}/'
    if not os.path.exists(folder):
        os.makedirs(folder)
        os.makedirs(f'{folder}/data')

    features_path = f'{folder}/data/features_window_dist_{dist_eps}.csv'
    print(features_path)
    if not os.path.exists(features_path):
        print('Getting Features')
        # features_all = fts.get_all_features(dataset, n_dirs=n_dirs, win=window, eps=seconds)
        features = fts.get_features_distance(dataset, eps=dist_eps)
    ...
    for nc in range(2, nc_size):
        file_name = f'{folder}/fishing_{nc}_{dist_eps}.csv'
        if not os.path.exists(file_name):
            ...
            sample_fishing.to_csv(f'{folder}/sample_fishing_{nc}_{dist_eps}.csv', index=False)
            dataset.to_csv(file_name, index=False)
        else:
            print('Reading clustering files')
            dataset = pd.read_csv(file_name)
            sample_fishing = pd.read_csv(f'{folder}/sample_fishing_{nc}_{dist_eps}.csv')
            test = dataset[dataset['trajectory'].isin([213, 117, 145, 26, 11])]
            if not os.path.exists(f'{folder}/test/'):
                os.makedirs(f'{folder}/test/')
            test.to_csv(f'{folder}/test/test_{nc}_{dist_eps}.csv', index=False)

        print('Computing metrics')
        DBI_l = davies_bouldin_score(features[['ma_d_acceleration', 'msum_d_roc']], dataset['labels'])
        print(f'DBI data = {DBI_l}')
        DBI_lp = davies_bouldin_score(features[['ma_d_acceleration', 'msum_d_roc']], dataset['labels_pos'])
