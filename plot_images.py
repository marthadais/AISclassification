...
features = pd.read_csv(features_path)
data_cl_obs = features[['ma_d_acceleration', 'msum_d_roc']]
file_name = f'{folder}/fishing_{nc}_{km}.csv'
dataset_obs = pd.read_csv(file_name)

fig = plt.figure(figsize=(10, 9))
plt.scatter(data_cl_obs['ma_d_acceleration'], data_cl_obs['msum_d_roc'], c=colors_bi[dataset_obs['labels_pos']], alpha=0.7)
plt.xlabel('MA of acceleration', fontsize=15)
plt.ylabel('MS of ROC', fontsize=15)
# plt.xticks(np.arange(-1.5, 1.7, step=0.2), fontsize=15)
# plt.yticks(np.arange(-800, 1600, step=100), fontsize=15)
...
fig = plt.figure(figsize=(10, 9))
plt.scatter(data_cl_obs['ma_d_acceleration'], data_cl_obs['msum_d_roc'], c=cmap(dataset_obs['labels']), alpha=0.7)
plt.xlabel('MA of acceleration', fontsize=15)
plt.ylabel('MS of ROC', fontsize=15)
# plt.xticks(np.arange(-1.5, 1.7, step=0.2), fontsize=15)
# plt.yticks(np.arange(-800, 1600, step=100), fontsize=15)
...
plt.plot(index_d, dist_d, marker="p", linestyle="-", linewidth=2,
         markersize=7, label='distance-based')
